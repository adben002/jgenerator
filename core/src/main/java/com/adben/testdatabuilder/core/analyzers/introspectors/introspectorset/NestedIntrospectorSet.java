...
import com.adben.testdatabuilder.core.analyzers.introspectors.introspector.NestedIntrospector;
 ...
 * <p>
 *
 * See {@link NestedIntrospectorSet#introspectors} for the default values.
 */
public class NestedIntrospectorSet extends AbstractIntrospectorSet {

    private static final Logger LOGGER = getLogger(lookup().lookupClass());

    public NestedIntrospectorSet() {
        super(ImmutableList.<Introspector>builder()
                .add(new NoOpIntrospector())
                .add(new GeneratedIntrospector(new DefaultGeneratorSet()))
                .add(new NestedIntrospector())
                .build());
