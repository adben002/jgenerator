...
public abstract class AbstractIteratorGenerator<T> extends AbstractGenerator<T> {
  ...
  protected AbstractIteratorGenerator(final Supplier<Generator> generatorParam,
      final Function<Class<?>, Cmd> cmdGeneratorParam) {

    super();
    this.generator = generatorParam;
    this.cmdGenerator = cmdGeneratorParam;
    LOGGER.trace("Constructor with value - generator: {}, cmdGenerator: {}",
        generatorParam,
        cmdGeneratorParam);
  }
  ...
  protected T getNextValue(final Type type) {
    ...
    if (generatedValue.isPresent()) {
      ...
      if (typeArg instanceof Class) {
        ...
        outputValue = this
            .createValues(type, foundClasses.get(this.counter++ % foundClasses.size()));
      } else {
