...
public final class DataBuilder {
  ...
  public DataBuilder build() {
    ...
    for (int i = 0; i < store().commands.size(); i++) {
      ...
      LOGGER.debug("Finished building cmd: {}. In {}ms.",
          cmd,
          buildStopwatch.elapsed(TimeUnit.MILLISECONDS));
    }
    LOGGER.debug("Finished building the cmds. In {}ms.",
        wholeBuildStopwatch.elapsed(TimeUnit.MILLISECONDS));
    return this;
  }
